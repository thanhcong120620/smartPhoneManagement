...
<c:forEach var="samsung" items="${resultProducts.rows}" varStatus="row">
 		
 		<c:if test='${samsung.product_brand == "SamSung" }'>
 		
<div class="general" class="ss1" name="ss1">
			
			<div class="image">			
    			<c:set scope="page" var="imgname" value="${samsung.product_img_source}"></c:set>
 				<a href="<c:url value="/infoProduct.jsp?action=product&product=${samsung.product_id}" />">
 					<img style="height: 225px; width:200px" src="${pageContext.request.contextPath}/media/phone/${imgname}" />
 				</a>   			
  			</div>
  
  			<div class="name">
    			<h3><c:out value="${samsung.product_name }"/></h3>
  			</div> 
  
  			<div class="price">
    			<h4><fmt:formatNumber value="${samsung.product_price }" maxFractionDigits="1" /></h4>
  			</div> 
